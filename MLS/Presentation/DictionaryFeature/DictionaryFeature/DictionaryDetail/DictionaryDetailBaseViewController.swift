...
private extension DictionaryDetailBaseViewController {
    ...
    func bindReportButton() {
        mainView.reportButton.rx.tap
            .bind { [weak self] in
                guard let self = self else { return }
                // dimmed background container 추가
                ...
                alert.mainTextLabel.numberOfLines = 0
                alert.mainTextLabel.textAlignment = .center
                
                // Alert를 dimmedView 위에 올리기
                dimmedView.addSubview(alert)
                ...
                alert.ctaButton.rx.tap
                    .bind { [weak dimmedView] in
                        UIView.animate(withDuration: 0.2, animations: {
                            dimmedView?.alpha = 0
                        }, completion: { _ in
                            let urlString = "https://forms.fillout.com/t/1JR9wiyv1Uus?providerId=xxxxx&item=주황버섯" // 건의 페이지
