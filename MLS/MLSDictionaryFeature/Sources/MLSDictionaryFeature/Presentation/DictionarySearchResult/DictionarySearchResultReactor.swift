...
public final class DictionarySearchResultReactor: Reactor {
    ...
    // MARK: - init
    public init(keyword: String?, dictionaryListAPIRepository: DictionaryListAPIRepository, recentSearchRepository: RecentSearchRepository) {
        ...
        let keywordChanges = mutation
            ...
            .flatMap { [weak self] keyword -> Observable<Mutation> in
                ...
                let countObservables = types.map { type in
                    self.dictionaryListAPIRepository.fetchSearchListCount(type: type, keyword: keyword)
                        .map { $0.count ?? 0 }
                }
