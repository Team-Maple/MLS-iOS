    ...
    public func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .viewWillAppear:
            return .merge([
                ...
                dictionaryDetailAPIRepository.fetchMonsterDetail(id: currentState.id).map { .setDetailData($0) },
                dictionaryDetailAPIRepository.fetchMonsterDetailDropItem(id: currentState.id, sort: nil).map { .setDetailDropItemData($0) },
                dictionaryDetailAPIRepository.fetchMonsterDetailMap(id: currentState.id).map { .setDetailMapData($0) }
            ])

        case let .selectFilter(type):
            return dictionaryDetailAPIRepository.fetchMonsterDetailDropItem(id: currentState.id, sort: type.sortParameter).map { .setDetailDropItemData($0) }
...
private extension MonsterDictionaryDetailReactor {
    func handleToggleBookmark() -> Observable<Mutation> {
        ...
        if isSelected, let bookmarkId = monster.bookmarkId {
            bookmarkObservable = bookmarkRepository
                .deleteBookmark(bookmarkId: bookmarkId)
        } else {
            bookmarkObservable = bookmarkRepository
                .setBookmark(resourceId: monster.monsterId, type: type)
                .map { Optional($0) }
        }
        ...
        return bookmarkObservable
            .flatMap { [weak self] newBookmarkId -> Observable<Mutation> in
                ...
                let refresh = self.dictionaryDetailAPIRepository
                    .fetchMonsterDetail(id: self.currentState.id)
