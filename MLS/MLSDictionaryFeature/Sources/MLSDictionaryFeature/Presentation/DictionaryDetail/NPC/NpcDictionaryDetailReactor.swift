...
public final class NpcDictionaryDetailReactor: Reactor {
    ...
    public func mutate(action: Action) -> Observable<Mutation> {
        switch action {
        case .filterButtonTapped:
            return .just(.navigateTo(.filter(currentState.questFilter)))
        case .viewWillAppear:
            return .merge([
                checkLoginUseCase.execute().map { .setLoginState($0) },
                dictionaryDetailAPIRepository.fetchNpcDetail(id: currentState.id).map { .setDetailData($0) },
                dictionaryDetailAPIRepository.fetchNpcDetailMap(id: currentState.id).map { .setDetailMaps($0) },
                dictionaryDetailAPIRepository.fetchNpcDetailQuest(id: currentState.id, sort: nil).map { .setDetailQuests($0) }
            ])
        case let .selectFilter(type):
            return dictionaryDetailAPIRepository.fetchNpcDetailQuest(id: currentState.id, sort: type.sortParameter).map { .setDetailQuests($0) }
        ...
        case .mapTapped(index: let index):
            return .just(.navigateTo(.detail(type: .map, id: currentState.maps[index].mapId)))

        case .questTapped(index: let index):
            return .just(.navigateTo(.detail(type: .quest, id: currentState.quests[index].questId)))
        }
    }
