    ...
    public func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .viewWillAppear:
            return .merge([
                ...
                dictionaryDetailAPIRepository.fetchQuestDetail(id: currentState.id).map { .setDetailData($0) },
                dictionaryDetailAPIRepository.fetchQuestDetailLinkedQuestsDetail(id: currentState.id).map { .setLinkedQuests($0) }
            ])
