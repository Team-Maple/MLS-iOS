...
public final class DictionaryNotificationReactor: Reactor {
    ...
    // MARK: - Init
    public init(fetchProfileUseCase: FetchProfileUseCase, checkNotificationPermissionUseCase: CheckNotificationPermissionUseCase, alarmRepository: AlarmRepository) {
    ...
    public func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .viewWillAppear:
            ...
            let notificationStream: Observable<Mutation> = Observable.concat([
                ...
                alarmRepository.fetchAll(cursor: nil, pageSize: 20)
                    .map { paged in
        ...
        case .loadMore:
            ...
            return Observable.concat([
                Observable<Mutation>.just(.setLoading(true)),
                alarmRepository.fetchAll(cursor: cursor, pageSize: 20)
                    .map { paged in
        ...
        case let .notificationTapped(index):
            ...
            return alarmRepository.setRead(alarmLink: notification.link)
                .andThen(
                    Observable.concat([
