...
public final class NotificationSettingReactor: Reactor {
    ...
    init(
        notificationRepository: NotificationPermissionRepository,
    ...
    public func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .viewWillAppear, .appWillEnterForeground:
            return notificationRepository.fetchAuthorizationStatus()
                .asObservable()
                .map { Mutation.setAuthorized($0) }
        case .backButtonTapped:
            return .just(.toNavigate(.dismiss))
        case .eventViewSwitch(let isAgree):
            return authRepository.updateNotificationAgreement(noticeAgreement: currentState.isAgreeNoticeNotification, patchNoteAgreement: currentState.isAgreePatchNoteNotification, eventAgreement: isAgree)
                .andThen(.just(.setEventNotification(isAgree)))
        case .noticeViewSwitch(let isAgree):
            return authRepository.updateNotificationAgreement(noticeAgreement: isAgree, patchNoteAgreement: currentState.isAgreePatchNoteNotification, eventAgreement: currentState.isAgreeEventNotification)
                .andThen(.just(.setNoticeNotification(isAgree)))
        case .patchNoteViewSwitch(let isAgree):
            return authRepository.updateNotificationAgreement(noticeAgreement: currentState.isAgreeNoticeNotification, patchNoteAgreement: isAgree, eventAgreement: currentState.isAgreeEventNotification)
